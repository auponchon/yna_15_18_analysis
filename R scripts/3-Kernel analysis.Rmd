...
loc.2015<-spTransform(loc.2015, CRS("+proj=utm +zone=45 +ellps=WGS84"))
 ...
 KUD.2015<- kernelUD(loc.2015[,2],same4all=T,h=70000,grid=1000)
    ...
    ver90.2015<-getverticeshr(KUDvol.2015 ,90)
    ver50.2015<-getverticeshr(KUDvol.2015 ,50)

    # export shapefiles
    writeOGR(ver90.2015, dsn="Data/NewlyCreatedData/Shapefiles",
             layer="KUD_90_day_for_2015", driver="ESRI Shapefile",overwrite_layer=T)
    writeOGR(ver50.2015, dsn="Data/NewlyCreatedData/Shapefiles",
             layer="KUD_50_day_for_2015", driver="ESRI Shapefile",overwrite_layer=T)
```
...
loc.2018<-spTransform(loc.2018, CRS("+proj=utm +zone=45 +ellps=WGS84"))
 ...
 KUD.2018<- kernelUD(loc.2018[,2],same4all=T,h=70000,grid=1000)
    ...
    ver90.2018<-getverticeshr(KUDvol.2018 ,90)
    ver50.2018<-getverticeshr(KUDvol.2018 ,50)

    writeOGR(ver90.2018, dsn="Data/NewlyCreatedData/Shapefiles",
             layer="KUD_90_day_for_2018", driver="ESRI Shapefile",overwrite_layer=T)
    writeOGR(ver50.2018, dsn="Data/NewlyCreatedData/Shapefiles",
             layer="KUD_50_day_for_2018", driver="ESRI Shapefile",overwrite_layer=T)
```


```{r Map kernel 2015 and 2018, echo=FALSE, fig.width=15,fig.height=8}
par(mfrow=c(1,2))
plot(ver90.2015,col=c("pink","yellow")[as.factor(ver90.2015$id)],
     border=c("pink","yellow")[as.factor(ver90.2015$id)],
     main="Kernel UD 2015")
plot(ver50.2015,col=c("purple","orange")[as.factor(ver50.2015$id)],
     border=c("purple","orange")[as.factor(ver50.2015$id)],add=T)


plot(ver90.2018,col=c("pink","yellow")[as.factor(ver90.2018$id)],
     border=c("pink","yellow")[as.factor(ver90.2018$id)],
     main="Kernel UD 2018")
plot(ver50.2018,col=c("purple","orange")[as.factor(ver50.2018$id)],
     border=c("purple","orange")[as.factor(ver50.2018$id)],add=T)
```
