...
diff.thres<-8 * 60    #max difftime in a trip (in min)
reso<-30             #time resolution for interpolation (in min)
...
```{r interpolation 2018}
 reso <- 30 * 60       #time resolution between 2 points in seconds
...
loc.interpolated.2018<-new.trip[,c("ID","Status","TravelNb","DateTime","Longitude","Latitude",
                                   "Distmax","Difftime","Distadj")]

#save(loc.interpolated.2018,file="NewlyCreatedData/clean_interp_loc_2018.RData")


summary.trips.interp1<-ddply(loc.interpolated.2018,.(ID,Status,TravelNb),summarize,DistmaxIndTrip=max(Distmax),
